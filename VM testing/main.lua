...
function love.draw()
 ...
 if debuger.t[1]%debuger.t[3]==1 or debuger.t[3]==1 then
  --for i=1,8 do
  -- print(i.." / "..System.memory[i])
  --end
  --print("")
  _Lo.g.captureScreenshot( tostring(math.floor(debuger.t[1]/debuger.t[3]) )..".png" )
 end
 -- ]]
...
function debug()
 ...
 _Lo.g.print("stack",0,42)
 
 _Lo.g.print("ptr# "..System.r.sta_ptr,0,48)
 
 _Lo.g.print("st "..tostring(System.r.sta_state),0,55)
 ...
 if not debuger.drawstate then
  ...
  for i=1,64 do
   ...
   if math.ceil(System.r.inst/3)==k then
    _Lo.g.setColor(1,1,.5)
   else
    _Lo.g.setColor(1,1,1)
   end
   
   str[i][1]=math.floor(k).."#"..actionNames[System.memory[k*3-2]+1]
   _Lo.g.print(str[i][1],36,l*5+10)
   str[i][2]=System.memory[k*3-1].." "..System.memory[k*3]
   _Lo.g.print(str[i][2],76,l*5+10)
  end
